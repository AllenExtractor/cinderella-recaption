"""
broadcast.py — Broadcast to all users/groups (OWNER only)
Commands:
  /broadcast  — Send a message to all users and groups (reply to content)
  /broadusers — View all registered users and groups
"""
...
def register_broadcast_handlers(bot: Client):
    ...
    # ── Auto-register every user who interacts ───────────────────────────────
    ...
    async def broadcast_handler(client: Client, message: Message):
        ...
        if not message.reply_to_message:
            return await message.reply_text(
                "📢 **Broadcast Mode**\n\n"
                "Please reply with content to broadcast.\n\n"
                "<blockquote>Broadcast goes to:\n"
                "• All users who ever started the bot\n"
